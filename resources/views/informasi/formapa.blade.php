        ...
        <div class="container" data-aos="fade-up" data-aos-delay="100">
            ...
            <div class="d-flex justify-content-end">
                <nav aria-label="Page navigation example">
                    <ul class="pagination">
                        <li class="page-item {{ $laporanUser1->onFirstPage() ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $laporanUser1->previousPageUrl() }}" tabindex="-1"
                               aria-disabled="{{ $laporanUser1->onFirstPage() ? 'true' : 'false' }}">Previous</a>
                        </li>

                        <!-- Page Numbers -->
                        @for ($i = 1; $i <= $laporanUser1->lastPage(); $i++)
                            <li class="page-item {{ $laporanUser1->currentPage() == $i ? 'active' : '' }}">
                                <a class="page-link" href="{{ $laporanUser1->url($i) }}">{{ $i }}</a>
                            </li>
                        @endfor

                        <!-- Next Button -->
                        <li class="page-item {{ $laporanUser1->hasMorePages() ? '' : 'disabled' }}">
                            <a class="page-link" href="{{ $laporanUser1->nextPageUrl() }}">Next</a>
                        </li>
                    </ul>
                </nav>
            </div>
